...
const state: {
  ...
  charts: { temp: any; moisture: any };
  phoneLocationReady: boolean;
} = {
  telemetry: null,
  file: null,
  entries: [],
  charts: { temp: null, moisture: null },
  phoneLocationReady: false,
};
...
const value = (id: string, text: string | number): void => {
  const element = document.getElementById(id);
  if (element) element.textContent = String(text);
};
const field = (id: string, text: string | number): void => {
  const element = document.getElementById(id) as HTMLInputElement | null;
  if (element) element.value = String(text);
};
...
function updateTelemetry(data: Telemetry): void {
  ...
  value(
    "hudSensorTag",
    `${data.temperature}°C | ${data.humidity}% RH | ${data.soil_moisture_pct}% SM`,
  );
  const status = data.diagnostics?.overall ?? "HEALTHY";
  value(
    "connectionStatusBadge",
    status === "CRITICAL" ? "ESP32 LINK ALERT" : "ESP32 LINK ONLINE",
  );
}
...
function clearTelemetry(): void {
  ...
  [
    ...
    "snapCurr",
    "hudSensorTag",
  ].forEach((id) => value(id, "Waiting for hardware data"));
...
function updateNodeOptions(nodeIds: string[]): void {
  ...
  select.innerHTML = nodeIds.length
    ? ""
    : '<option value="">No hardware node online</option>';
...
function updateCharts(history: Telemetry[]): void {
  ...
  const emptyMessage =
    history.length < 2 ? "Waiting for real ESP32 data..." : "";
  document
    .querySelectorAll(".chart-empty-state")
  ...
  state.charts.temp = new Chart(document.getElementById("tempChart"), {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Air Temp",
          data: history.map((item) => Number(item.temperature)),
          borderColor: "#f59e0b",
          tension: 0.35,
        },
        {
          label: "Soil Temp",
          data: history.map((item) => item.soil_temperature),
          borderColor: "#10b981",
          tension: 0.35,
        },
      ],
  ...
  state.charts.moisture = new Chart(
    document.getElementById("moisturePowerChart"),
    {
      type: "line",
      data: {
        labels,
        datasets: [
          {
            label: "Humidity",
            data: history.map((item) => Number(item.humidity)),
            borderColor: "#06b6d4",
            tension: 0.35,
          },
          {
            label: "Soil Moisture",
            data: history.map((item) => item.soil_moisture_pct),
            borderColor: "#3b82f6",
            tension: 0.35,
          },
          {
            label: "Power / 10",
            data: history.map((item) => item.power_mw / 10),
            borderColor: "#8b5cf6",
            tension: 0.35,
          },
        ],
...
async function pollTelemetry(): Promise<void> {
  try {
    ...
    const response = await fetch(
      selectedNode
        ? `/api/sensor-data?node_id=${encodeURIComponent(selectedNode)}`
        : "/api/sensor-data",
    );
    const data = (await response.json()) as {
      latest: Telemetry | null;
      history: Telemetry[];
      active_nodes?: Array<{ node_id: string }>;
    };
...
function renderEntries(entries: DatasetEntry[]): void {
  ...
  body.innerHTML = entries.length
    ? ""
    : '<tr><td colspan="11">No samples saved yet.</td></tr>';
...
function initMap(): void {
  ...
  marker.on("dragend", () => {
    ...
    ($("inputLat") as HTMLInputElement).value = point.lat.toFixed(4);
    ($("inputLng") as HTMLInputElement).value = point.lng.toFixed(4);
  });
}
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      if (altitude === null || !Number.isFinite(altitude)) {
        toast(
          "Phone did not provide real altitude; enable precise location",
          true,
        );
        return;
      }
...
function init(): void {
  ...
  const handleImage = (input: HTMLInputElement): void => {
    state.file = input.files?.[0] ?? null;
    if (state.file) {
      const preview = $("leafImagePreview") as HTMLImageElement;
  ...
  $("btnOpenCamera").addEventListener("click", (event) => {
    event.stopPropagation();
    cameraInput.click();
  });
  $("dropzoneContainer").addEventListener("click", (event) => {
    if (!(event.target as HTMLElement).closest("button")) cameraInput.click();
  });
  ...
  $("datasetRecordForm").addEventListener("submit", async (event) => {
    ...
    form.append("node_id", ($("nodeSelect") as HTMLSelectElement).value);
    [
      "inputCropType:crop_type",
      "inputGrowthStage:growth_stage",
      "inputDiseaseLabel:disease_label",
      "inputSeverity:severity",
      "inputLat:gps_latitude",
      "inputLng:gps_longitude",
      "inputAlt:gps_altitude",
      "inputNotes:notes",
    ].forEach((item) => {
      const [id, key] = item.split(":");
      form.append(key, ($(id) as HTMLInputElement | HTMLSelectElement).value);
    });
    if (state.telemetry) {
      ...
      form.append(
        "soil_temperature_c",
        String(state.telemetry.soil_temperature),
      );
    ...
    const response = await fetch("/api/dataset/entry", {
      method: "POST",
      body: form,
    });
    if (!response.ok) {
