...
function ensureDatasetFiles(): void {
  ...
  if (!fs.existsSync(METADATA_CSV))
    fs.writeFileSync(METADATA_CSV, `${headers.join(",")}\n`, "utf8");
  if (!fs.existsSync(SENSOR_DATA_JSON))
    fs.writeFileSync(SENSOR_DATA_JSON, "[]\n", "utf8");
}
  ...
  return rows
    .map((values) => {
      const entry = Object.fromEntries(
        columnNames.map((key, index) => [key, values[index] ?? ""]),
...
export function createEntry(
  ...
  if (!telemetry)
    throw new Error(`No live telemetry available for node ${nodeId}`);
  ...
  if (![latitude, longitude, altitude].every(Number.isFinite))
    throw new Error("Phone GPS latitude, longitude, and altitude are required");
  ...
  const entry: DatasetEntry = {
    ...
    node_id: nodeId,
    crop_type: fields.crop_type || "Rice (Oryza sativa)",
    growth_stage: fields.growth_stage || "Vegetative",
    disease_label: fields.disease_label || "Healthy",
    ...
    soil_temperature_c: telemetry.soil_temperature,
    soil_moisture_raw: moistureRaw,
    soil_moisture_pct: calculateMoisturePercent(moistureRaw),
    voltage_v: voltage,
    current_ma: current,
    power_mw: Number((voltage * current).toFixed(2)),
    notes: fields.notes || "Rice leaf captured in Rajshahi field",
  };
  fs.appendFileSync(
    METADATA_CSV,
    `${headers.map((key) => csvEscape(entry[key as keyof DatasetEntry])).join(",")}\n`,
    "utf8",
  );
  const jsonEntries = JSON.parse(
    fs.readFileSync(SENSOR_DATA_JSON, "utf8"),
  ) as unknown[];
  ...
  return {
    ...entry,
    image_url: `/dataset-images/${encodeURIComponent(entry.image_filename)}`,
  };
}

export function datasetCsvPath(): string {
  return path.resolve(METADATA_CSV);
}
export function datasetJsonPath(): string {
  return path.resolve(SENSOR_DATA_JSON);
}
export { DATASET_DIR };
